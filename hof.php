...
{
    addnav("Sympathie","hof.php?op=symp&subop=$subop");
}
...
output("<form action='hof.php?op=" . $_GET['op'] . "&subop=" . $subop . "&page=1' method='POST'>Helden suchen: <input name='search' value='" . $_POST['search'] . "'><input type='submit' name='suchen' class='button' value='Suchen'></form>");
...
{
    $sql = "SELECT accounts.name,$sexsel AS data1,r.colname AS data2 FROM accounts LEFT JOIN races r ON r.id=race WHERE locked=0 ".$max_age.$max_su." ORDER BY charm $order, level $order, experience $order, acctid $order LIMIT $limit";
    $adverb = "schönsten";
    if ($_GET['subop'] == "least")
    {
        $adverb = "hässlichsten";
...
{
    $sql = "SELECT accounts.name,level AS data2 ,r.colname as data1 FROM accounts LEFT JOIN races r ON r.id=race WHERE locked=0 ".$max_age.$max_su." ORDER BY ((maxhitpoints/30)+(attack*1.5)+(defence)) $order, level $order, experience $order, acctid $order LIMIT $limit";
    $adverb = "stärksten";
    if ($_GET['subop'] == "least")
    {
        $adverb = "schwächsten";
...
else if ($_GET['op']=="punch")
{
    ...
    $sql = "SELECT accounts.name,punch AS data1,r.colname AS data2 FROM accounts LEFT JOIN races r ON r.id=race WHERE locked=0 ".$max_age.$max_su_punch." ORDER BY data1 $order, level $order, experience $order, acctid $order LIMIT $limit";
    $adverb = "härtesten";
    if ($_GET['subop'] == "least")
    {
        $adverb = "armseligsten";
...
{
    $sql = "SELECT accounts.name,level AS data1 FROM accounts WHERE locked=0 ".$max_age.$max_su." ORDER BY resurrections $order, level $order, experience $order, acctid $order LIMIT $limit";
...
{
    $sql = "SELECT accounts.name,deathpower AS data1,location,$loginsel AS data2,laston,$alivesel AS data3,activated FROM accounts WHERE locked=0 ".$max_age.$max_su." ORDER BY deathpower $order, level $order, experience $order, acctid $order LIMIT $limit";
    $adverb = "fleissigste";
    if ($_GET['subop'] == "least")
    {
        $adverb = "faulste";
    ...
    else
    {
        ...
        else
        {
            for ($i=0; $i<db_num_rows($result); $i++)
            {
                ...
                output("</td><td>",true);
                output($row['data3']?"`1Lebt`0":"`4Tot`0");
                output("</td></tr>",true);
            }
        }
        output("</table>", true);
...
{
    $sql = "SELECT accounts.name,battlepoints AS data1,dragonkills AS data2 FROM accounts WHERE locked=0 ".$max_age.$max_su." ORDER BY battlepoints $order, dragonkills $order, acctid $order LIMIT $limit";
    $adverb = "besten";
    if ($_GET['subop'] == "least")
    {
...
{
    $sql = "SELECT accounts.name, DATEDIFF(NOW(),laston) AS data1,dragonkills AS data2 FROM accounts WHERE locked=0 AND DATEDIFF(NOW(),laston) > 3 AND dragonkills>0 ORDER BY data1 $order, dragonkills $order, acctid $order LIMIT $limit";
    $adverb = "am längsten";
    if ($_GET['subop'] == "least")
    {
        $adverb = "am kürzesten";
