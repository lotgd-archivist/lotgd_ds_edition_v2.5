...
switch($op) {
	...
	case 'show_guild_bio':
		...
		if(strpos($g_ret_page,'show_guild_history') === false) {
		...
		if(empty($_GET['gid'])) {
			redirect( (empty($str_ret) ? 'news.php' : $str_ret) );
		}
...
/*		if(su_check(SU_RIGHT_EDITORGUILDS)) {
			addnav('MOD-Aktionen');
			addnav('X?Gildeneditor','dg_su.php?op=edit&gid='.$_GET['gid']);
		}
*/
		break;
	...
	case 'show_guild_history':
		...
		addnav('Zurück','dg_main.php?op=show_guild_bio&gid='.$_GET['gid']);
		...
		switch($subop) {
			...
			case 'leave':
				...
				if($_GET['act'] == 'ok') {
					...
					output('`8Mit einem leicht flauen Gefühl im Bauch überreichst du dein Kündigungsschreiben. Seufzend denkst du an all die schönen Stunden zurück, die du hier verbracht hast, lässt deinen Blick zum letzten Mal durch die Halle schweifen - und machst dich dann auf den Weg zurück ins Dorf.');
			...
			case 'private':
				...
				$sql = 'SELECT i.id,i.owner,a.name,a.acctid FROM items i LEFT JOIN accounts a ON a.acctid=i.owner WHERE i.tpl_id="gldprive" AND i.value1='.$gid.' ORDER BY a.name ASC,a.acctid ASC';
				$res = db_query($sql);
				$counter = 0;
				$ids = '';
