...
if ($_GET[op]=="drin")
{
	...
	addnav("Haus abreißen","suhouses.php?op=raze&id=$_GET[id]"); // Leeres Grundstück erzeugen
	addnav("~~~");
	addnav("Haus löschen","suhouses.php?op=destroy&id=$_GET[id]"); // Vorsicht!!
	addnav("~~~");
...
else if ($_GET[op]=="raze")
{
	...
	{
		$sql="UPDATE houses SET owner=0,housename='Leere Fläche',gold=0,gems=0,status=6,description='' WHERE houseid=$_GET[id]";
		db_query($sql);
	...
	else
	{
		...
		addnav("Einreißen","suhouses.php?op=raze&subop=confirmed&id=$_GET[id]");
	}
	addnav("Hausmeister","suhouses.php");
}
