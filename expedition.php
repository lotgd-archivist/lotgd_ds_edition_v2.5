...
Ganz am Anfang hat dir die Expeditionsleitung mehrere Gebiete auf der Karte gezeigt, die noch niemand untersucht hat. Allerdings wird vermutet, dass im Buschland am meisten zu finden ist. Dementsprechend gering wird hier deine Belohnung ausfallen. Dagegen sind die Leiter sich sicher, dass die Felsenwüste fast keinen Nutzen für die Expedition hat. Solltest du dort wirklich etwas finden, werden sie dir sicher mehr Gold und Edelsteine überreichen.`n');
    ...
    case 'claim2' :
        ...
        if ($session['user']['castleturns']>0)
        {
            ...
            {
                output('`2Nach dem deine Arbeiten beendet sind musst du feststellen, dass dieses Stück Land vollkommen unbrauchbar ist.`n');
            }
        }
        else
        {
            output('`2Du kannst heute keine Analyse mehr durchführen!`n');
        }
    ...
    case 'letter' :
        ...
        if ($_GET['act']=="")
        {
            ...
            else
            {
                ...
                {
                    output("Du denkst nochmal über deine Ehe mit ".($session['user']['sex']?"`^Seth`0":"`5Violet`0")." nach und überlegst, ob du ".($session['user']['sex']?"ihn":"sie")." in der Kneipe besuchen sollst, oder für wen du diese Ehe aufs Spiel setzen würdest.`n");
                }
                if ($session['user']['charme']==4294967295)
                {
                    output("Du überlegst dir, dass du dir mal wieder etwas Zeit für ".($session['user']['sex']?"deinen Mann":"deine Frau")." nehmen solltest. Während du ".($session['user']['sex']?"ihn":"sie")." im Garten suchst, stellst du aber fest, dass der Rest der ".($session['user']['sex']?"Männer":"Frauen")." hier auch nicht zu verachten ist.`n");
                }
            ...
            if (db_num_rows($result)>0)
            {
                ...
                if (($session['user']['marriedto']==4294967295 || $session['user']['charisma']==4294967295) && ($row['marriedto']==4294967295 || $row['charisma']==4294967295))
                {
                    ...
                    if ($session['user']['marriedto']==$row['acctid'] && $session['user']['acctid']==$row['marriedto'])
                    {
                        ...
                        systemmail($row['acctid'],"`%Ein Brief!`0","`&{$session['user']['name']}
                        `6 hat dir einen sehnsuchtsvollen Brief aus den fernen, dunklen Landen geschrieben.".$more);
                    }
                    ...
                    case 3 :
                        ...
                        $session['user']['seenlover']=1;
                        systemmail($row['acctid'],"`%Ein Brief!`0","`&{$session['user']['name']}
                        `6 hat dir einen sehnsuchtsvollen Brief aus den fernen, dunklen Landen geschrieben. Da du weißt wie riskant es ist, dass dein Partner diesen Brief zu sehen bekommt lässt du ihn nach mehrmaligem Lesen schnell wieder verschwinden.".$more);
                        break;
                    case 4:
                        output("`%Du schreibst `6$row[name]`% einen romantischen Brief aus der Ferne. Da ihr beide verheiratet seid, hoffst du das dieser Brief nicht in falsche Hände gerät.");
                        ...
                        systemmail($session['user']['marriedto'],"`\$Scheidung!`0","`6Du hast einen Liebesbrief von `&{$session['user']['name']}
                        `6 zugestellt bekommen, der eigentlich für `&{$row['name']}
                        ...
                        $session['user']['seenlover']=1;
                        systemmail($row['acctid'],"`%Ein Brief!`0","`&{$session['user']['name']}
                        `6 hat dir einen sehnsuchtsvollen Brief aus den fernen, dunklen Landen geschrieben. Leider hat ihn der schusselige Bote ".($session['user']['sex']?"ihrem Mann übergeben, der ":"seiner Frau übergeben, der ")." sich natürlich sofort scheiden ließ. Jedoch hat ".($session['user']['sex']?"ihr Ex-Mann ":"seine Ex-Frau ")."dir diesen Brief wortlos zukommen lassen.".$more);
                        break;
                    }
                }
...
`7Stufe 5: `$Du wurdest sehr schwer verletzt und warst dem Tode nah. Doch dank der Hilfe deiner Kameraden und des Lagerarztes hast du nun das Schlimmste überstanden. Dennoch wird es etwas dauern bis du wieder kämpfen kannst.`0`n`n');
    ...
    case 'pools' :
        ...
        if ($_POST['talkline'] && !$GLOBALS['doublepost'] && e_rand(1,5)==3)
        {
            addnav('Aktionen');
    ...
    case 'give_medal' :
        ...
        for ($i=0; $i<$max; $i++)
        {
            ...
            if ($session['user']['loggedin'])
            {
                output("<a href=\"mail.php?op=write&to=".rawurlencode($row['login'])."\" target=\"_blank\" onClick=\"".popup("mail.php?op=write&to=".rawurlencode($row['login'])."").";return false;\"><img src='images/newscroll.GIF' width='16' height='16' alt='Mail schreiben' border='0'>",true);
            }
            if ($session['user']['loggedin'])
            {
                output("<a href='expedition.php?op=give_medal2&char=".$row['acctid']."'>",true);
            }
            if ($session['user']['loggedin'])
            {
                addnav("","expedition.php?op=give_medal2&char=".$row['acctid']);
            }
            output("$row[name]`0");
            if ($session['user']['loggedin'])
            {
    ...
    case 'invite2' :
        ...
        db_query($sql) or die(db_error(LINK));
        systemmail($row['acctid'],"`%DDL : Einladung ins Zelt von ".$session['user']['login']."`%!`0","`&{$session['user']['name']}
        `6 wünscht dich in ".($row['sex']?"ihrem ":"seinem ")." Zelt zu sprechen - unverzüglich und allein...");
        addnav('Zurück','expedition.php?op=mytent');
        break;
...
gerade in Händen hält und nimmt Haltung an. Dir wird die Lage gemeldet und alle blicken dich erwartungsvoll an.`n`n');
        ...
        board_view('expi_guard',$int_del,'Folgende Botschaften wurden von der Expeditionsleitung hier verkündet:','Es wurden noch keine Botschaften verkündet!',true,true);
    ...
    case 'wastes' :
        ...
        else
        {
            ...
            if ($xstate<11)
            {
                ...
                if($_POST['talkline'] && !$GLOBALS['doublepost'])  // Kommentare wurde geschrieben
                {
                    if ($DDL_opps<=0)
                    {
                        $amount++;
...
WHERE DDL_tent=".$session['user']['acctid'];
    ...
    if ($battle)
    {
        ...
        if ($victory)
        {
            ...
            if (e_rand(1,2)==1)
            {
                $sql="INSERT INTO commentary(postdate,section,author,comment) VALUES(now(),'expedition_wastes',".$session['user']['acctid'].",'/me `@hat `^".$badguy['creaturename']."`@ nieder gestreckt.`0')";
            }
            else
            {
        ...
        else if ($defeat)
        {
            ...
            if (e_rand(1,2)==1)
            {
                $sql="INSERT INTO commentary(postdate,section,author,comment) VALUES(now(),'expedition_wastes',".$session['user']['acctid'].",'/me `4wird von `^".$badguy['creaturename']."`4 niedergeschmettert und bleibt regungslos liegen.`0')";
            }
            else
            {
