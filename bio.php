...
$result = db_query("SELECT
					...
					login,loggedin,laston,accounts.name,level,sex,title,specialty,hashorse,acctid,age,marriedto,pvpflag,charisma,weapon,
...
// Sympathie
if ($session['user']['acctid']!=$row['acctid'] && (bool)getsetting('symp_active','0')) 
{
...
if ($session['user']['superuser']>0)
{
	...
	if(su_check(SU_RIGHT_PRISON))
	{
		if ($row['imprisoned']==0)
		{
			addnav("Einkerkern","bio.php?char=".urlencode($str_char)."&act=prison&ret=".$_GET['ret']."&op=".$_GET['op']);
		}
		else
		{
			addnav("Befreien","bio.php?char=".urlencode($str_char)."&act=free&ret=".$_GET['ret']."&op=".$_GET['op']);
		}
	}
	...
	{
		if($row['activated'] != USER_ACTIVATED_MUTE)
		{
			addnav('Knebeln','bio.php?char='.urlencode($str_char).'&act=mute&ret='.$_GET['ret'].'&op='.$_GET['op']);
		}
		else
		{
			addnav('Entknebeln','bio.php?char='.urlencode($str_char).'&act=demute&ret='.$_GET['ret'].'&op='.$_GET['op']);
		}
	}

	if(su_check(SU_RIGHT_LOCKHTML))
	{
		if($rowextra['html_locked'] == 0)
		{
			addnav('HTML sperren','bio.php?char='.urlencode($str_char).'&act=lock_html&ret='.$_GET['ret'].'&op='.$_GET['op']);
		}
		else
		{
			addnav('HTML Entsperren','bio.php?char='.urlencode($str_char).'&act=unlock_html&ret='.$_GET['ret'].'&op='.$_GET['op']);
		}
	}
	addnav('Diskussion','su_userdiscu.php?op=new&id='.$row['acctid'].'&r='.urlencode(calcreturnpath()));
...
if (($session['user']['profession']==PROF_GUARD_HEAD || $session['user']['profession']==PROF_GUARD)) {
	addnav("Stadtwache");
	...
	if($strength < $row['strength']) {
		$str_guard_info .= 'Diese Person ist leider zu stark, als dass du sie in einem Kampf überwinden könntest..';
		$bool_ok = false;
	}
	if($rowextra2['profession_tmp'] >= getsetting('guard_max_imprison',2)) {
		$str_guard_info .= 'Für heute hast du schon zu viele Verbrecher abgeführt.. noch eine Verhaftung würdest du kaum durchstehen.';
		$bool_ok = false;
	}
...
// Wenn Nachricht vorhanden
if(!empty($session['message'])) {
	output('`n`b'.$session['message'].'`b`n');
...
$str_guard_info = (!empty($session['guard_info']) ? $session['guard_info'] : '');
if(!empty($str_guard_info)) {
	unset($session['guard_info']);
	output('`n`$'.$str_guard_info);
}
...
elseif (substr($row['name'],0,12)=="`tSandgeist ")
{
	output('`n`n<img src="images/sandgeist.png" ',true);
}
...
if ($row['pvpflag']==PVP_IMMU) {output("`4`iSteht unter besonderem Schutz`i");}
...
if ($_GET[act]=="symp") {
	...
	if (($rowsy['symp_given']==0) && ($rowsy['symp_votes']<$maxsymp) && (($session['user']['dragonkills']>0) && (getsetting("symp_dk_lock","1")==1)))
	{
		...
		// Wenn setting Stimmbündelung einschränkt
		if(getsetting('symp_per_acc',10) < $maxsymp) {
			// Wenn max. Anzahl an Symp.punkten auf diesen Account noch nicht überschritten
			$sql = 'SELECT COUNT(*) AS c FROM sympathy_votes WHERE from_user='.$session['user']['acctid'].' AND to_user='.$row['acctid'];
			$count = db_fetch_assoc(db_query($sql));
			
			if($count['c'] >= getsetting('symp_per_acc',10)) {
				$session['message'] = '`$Du hast diesem Charakter bereits genug Sympathiepunkte gegeben. So gerne kannst du ihn ja gar nicht haben.`0';
...
if($_GET['act'] == 'guard_prison') {
	...
	if(!$row['loggedin']) {
		$session['guard_info'] = $row['name'].' `$ist nicht mehr eingeloggt und kann somit nicht verhaftet werden!';
		redirect("bio.php?char=".urlencode($str_char)."&act=0&ret=".$_GET['ret']."&op=".$_GET['op']);
	}
	
	if($row['imprisoned']) {
		$session['guard_info'] = $row['name'].' `$sitzt bereits im Kerker!';
		redirect("bio.php?char=".urlencode($str_char)."&act=0&ret=".$_GET['ret']."&op=".$_GET['op']);
	}
	...
	if(empty($ok['section'])) {
		$session['guard_info'] = $row['name'].' `$befindet sich gar nicht am selben Ort wie du!';
		redirect("bio.php?char=".urlencode($str_char)."&act=0&ret=".$_GET['ret']."&op=".$_GET['op']);
	}
	...
	$str_ihn_sie = ($row['sex']==0)?'ihn':'sie';

	$msg = ': `5überwindet '.$row['name'].'`5, packt '.$str_ihn_sie.' mit eisernem Griff und führt '.$str_ihn_sie.' Richtung Kerker!';
	...
	$sql = 'UPDATE account_extra_info SET profession_tmp=profession_tmp+1 WHERE acctid='.$session['user']['acctid'];
	db_query($sql);
	
	$str_pro = $session['user']['sex'] ? 'ihrer' : 'seiner';
	
	debuglog('nutzte seine Stadtwachenfähigkeiten und verhaftete ',$row['acctid']);
	addnews($session['user']['name'].'`# hat '.$row['name'].'`# in '.$str_pro.' Eigenschaft als Stadtwache festgenommen und in den Kerker gesteckt!');

	systemmail($row['acctid'],'`$Verhaftet!',$session['user']['name'].'`$ hat dich soeben in '.$str_pro.' Eigenschaft als Stadtwache festgenommen. Du darfst nun einen Tag im Kerker verbringen!');
...
if ($_GET[act]=="prison") {
	...
	$sql = "UPDATE accounts SET imprisoned=0,location=0,restatlocation=0 WHERE acctid = ".$row['acctid']."";
	db_query($sql) or die(sql_error($sql));
	...
	$profession=$row['profession'];
	if ($profession>=41 && $profession < 50) { $profession=0; }
	else {
		$profession = $row['profession'];
	}

	$sql = "UPDATE accounts SET expedition=0,profession=$profession WHERE acctid = ".$row['acctid']."";
	db_query($sql) or die(sql_error($sql));
