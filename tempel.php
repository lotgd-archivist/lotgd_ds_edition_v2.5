...
if($chance < 100 && $_GET['op'] == '' && strpos($g_ret_page,'tempel') === false ) {
	output('`gAls du dich der Pforte des heiligen Tempels näherst, spürst du ein ungutes Ziehen in der linken unteren Bauchgegend.`n
			...
			DU bist ja ein '.$arr_race['colname'].'`g! Und deine Rasse verhindert dir wohl den Zutritt zu diesem Tempel der guten Göttter. Mist..`n
...
switch ($op) {
	...
	case '':
		...
		output("Auf der rechten Seite, hinter den Säulen entdeckst einen kleineren Altar, der für Opfer gedacht zu sein scheint.`n`n");
	...
	case 'bewerben':
		...
		if($session['user']['dragonkills'] < getsetting('priestreq',15)) {
			output("Du musst mindestens ".getsetting('priestreq',15)."mal den grünen Drachen getötet haben, um Priester werden zu können!");
...
//			hochz(getsetting('temple_id1',0),getsetting('temple_id2',0),true);
	...
	case 'opfer_ok':
		...
		break;
	case 'gardenmaze':
		output('`2Du atmest ruhig ein und aus, ein und aus... fühlst Deine Entspannung wachsen. Schließlich bist Du den Göttern ganz nah und bietest Ihnen ein Opfer.`nSie nehmen dir 10% deiner Lebenskraft und führen dich an einen verlassenen Ort.');
