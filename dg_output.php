...
function dg_show_builds ($gid,$actions=false,$admin_mode=0) {
	...
	foreach($dg_builds as $k=>$b) {
		...
		$out[$which] .= '<tr class="'.($count%2?"trlight":"trdark").'"><td>`b'.$b['color'].$b['name'].'`0`b</td>
						...
						<td>&nbsp;'.number_format($costs['gp'],0,null,' ').'&nbsp;</td>
						<td>&nbsp;'.number_format($costs['gold'],0,null,' ').'&nbsp;</td>
						<td>&nbsp;'.number_format($costs['gems'],0,null,' ').'&nbsp;</td>
						<td>&nbsp;'.number_format($costs['days'],0,null,' ').' Tage&nbsp;</td>';
...
function dg_show_transfer_list ($gid,$acctid=0,$old=false) {
	...
	$out = '<table border="0" cellpadding="5" cellspacing="3"><tr class="trhead"><td>`bName`b</td><td>`bGold ein-/ausgezahlt`b</td><td>`bEdelsteine ein-/ausgezahlt`b</td><td>`bInsigniensplitter`b</td><td>`bIst Mitglied?`b</td></tr>';
	
	if(!$acctid) {
		
		if(!is_array($guild['transfers'])) {$out .= '<tr><td colspan="5">`iKeine Transfers vorhanden!</td></tr>';}
			...
			foreach($guild['transfers'] as $k=>$t) {
				...
				if($name != '') {
					...
					$out .= '<tr class="'.($i%2?"trlight":"trdark").'"><td>'.$name.'</td><td align="center">'.
