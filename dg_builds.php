	...
	//						if($rowe['itemsout'] + $int_val > getsetting('maxitemsout',8000) && $rowe['itemsout'] > 0) {
			...
			case 'bibli':
				...
				if($_GET['act'] == 'ok') {
					...
					addnav('Zur Bibliothek','dg_main.php?op=in&subop=buildings&building_op=bibli');
					...
					output('`n`n');
					viewcommentary('guild-'.$gid.'_lib','Sprechen:',25,'spricht');
			...
			case 'altar':
				...
				if($_GET['act'] == 'ok') {
					...
					if($points > $guild['points']) {
						...
						$sql = 'UPDATE accounts SET hitpoints=ROUND(hitpoints*'.$impr.') WHERE guildid='.$gid.' AND guildfunc!='.DG_FUNC_APPLICANT.''; 
						db_query($sql);
						...
						foreach($arr_animals as $animal => $a) {
							if($_POST[$animal.'_taufe'] && $_POST[$animal] != $a['name']) {
								if($guild['points'] >= DG_STALL_RENAME_COST) {
									$str_name = strip_appoencode(substr($_POST[$animal],0,40),2);
									//$str_name = htmlentities($str_name);
									...
									$session['msg'] .= '`2Mit dem Spruch, den du vom guten alten Merrick aufgeschnappt hast, versuchst du '.$a['oname'].'`2 zu taufen:`n
