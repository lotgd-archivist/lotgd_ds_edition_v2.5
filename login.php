...
else if ($_GET['op']=="logout")
{
    ...
    if ($session['user']['loggedin'])
    {
        ...
        $sql = "UPDATE accounts SET location=".$int_loc.",loggedin=0,restatlocation=".$int_restatloc.",output='Ausgeloggt in Loc ".$int_loc.", Restatloc ".$int_restatloc." am ".date('d.m.Y H:i:s')."' WHERE acctid = ".$session['user']['acctid'];
        db_query($sql) or die(sql_error($sql));
    }
