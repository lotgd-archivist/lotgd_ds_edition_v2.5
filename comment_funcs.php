...
if($_GET['callmod']) {
	...
	if( time() - $session['modcalled'] < 300 && $session['user']['superuser'] == 0) {
		...
		if(db_num_rows($res)) {		
			...
			$msg = 'Beschwerde über Kommentare der letzten Stunde in Sektion '.$_GET['section'].' ('.$ret.'):\n\n';
	...
	if($act == 'send') {
		...
		// Wenn max. Anzahl noch nicht zur Verfügung steht
		if($int_count < $int_maxcount) {
		    $str_count_radio .= ','.$int_maxcount.',Alle vorhandenen ('.$int_maxcount.')';			    
		}
