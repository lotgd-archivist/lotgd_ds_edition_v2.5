...
if ($_GET[act]=="takekey")
{
    ...
    else
    {
        ...
        `2 hat den Schlüssel zu Haus Nummer `b$row[houseid]`b ($row[housename]`2) zurückverlangt. ".
        	$str_mail_plus
        	);
        ...
        $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'house-".$row[houseid]."',".$session[user][acctid].",'/me `^nimmt $row2[name]`^ einen Schlüssel ab. $str_comment_plus')";
    ...
    addnav("H?Zurück zum Haus","inside_houses.php");
...
else if ($_GET[act]=="givekey")
{
    ...
    if (!$_POST['ziel'])
    {
        ...
        if (db_num_rows($result)>0)
        {
            ...
            output("`n`nWenn du einen Schlüssel vergibst, wird der Schatz des Hauses gemeinsam genutzt. Du kannst einem Mitbewohner jederzeit den Schlüssel wieder wegnehmen. ");
        	if(getsetting('housetrsshare',1)) {
    ...
    else
    {
        ...
        if ($_GET['subfinal'])
        {
            ...
            else
            {
				...
				output("`2Du übergibst `&$row2[name]`2 einen Schlüssel für dein Haus. Du kannst den Schlüssel zum Haus jederzeit wieder wegnehmen");
				if(getsetting('housetrsshare',1)) {
...
und dann noch einmal um einen Ausbau zu ersuchen!');
    ...
    else if ($_GET[form]=="done")
    {
        ...
        case 10 :
            output("`&Dein neues Anwesen wird viel mehr an Reichtümern aufnehmen können als dein altes Haus.`n`7Du erhältst `@5`7 weitere Schlüssel!");
        ...
        case 14 :
            output("`&Deine neue Villa wird noch mehr Reichtümer horten können als das Anwesen.`n`7Du erhältst `@3`7 weitere Schlüssel!");
        ...
        case 17 :
            output("`&Dein neues Gasthaus wird dir und deinen Mitbewohnern eine willkommene Möglichkeit zur Rast und Stärkung bieten.`n`7Du erhältst `@3`7 weitere Schlüssel!");
        ...
        case 37 :
            output("`&Dein neues Kellergewölbe ist so gestaltet, dass es einen Bewohner mehr aufnehmen kann, auch der Komfort wurde ein klein wenig gehoben. Dennoch ist es immer noch nicht die schönste Art zu übernachten.`n`7Du erhältst `@zwei`7 weitere Schlüssel!`n`&");
        ...
        else
        {
            ...
            if ($row[gems]>=$upgems && $row[gold]>=$upgold)
            {
                ...
                case 5 :
                    ...
                    addnav("h?Muss ich mir noch überlegen","inside_houses.php");
                    break;
                ...
                case 31 :
                    ...
                    addnav("K?Ein Kellergewölbe!","inside_houses.php?act=upgrade&form=done&choice=37");
                    addnav("h?Muss ich mir noch überlegen","inside_houses.php");
                    break;
                ...
                case 41 :
                    ...
                    addnav("n?Muss ich mir noch überlegen","inside_houses.php");
                    break;
                ...
                case 71 :
                    ...
                    addnav("G?Ein Gefängnis!","inside_houses.php?act=upgrade&form=done&choice=74");
                ...
                case 91 :
                    ...
                    addnav("S?Ein Söldnerlager!","inside_houses.php?act=upgrade&form=done&choice=97");
                    addnav("h?Muss ich mir noch überlegen","inside_houses.php");
                    break;
...
else if ($_GET[act]=="removeupg")
{
    ...
    {
        output("`7Deine vergrößerte Schatzkammer wird entfernt. Alles Gold und alle Edelsteine, die danach zuviel in deiner Truhe sind gehen `4UNWIEDERBRINGLICH VERLOREN!`7`n");
    }

    output("`@Dein Haus wird wieder ein gewöhnliches Wohnhaus sein. Gekaufte Privatgemächer werden entfernt. Zusätzliche Schlüssel der Villa werden entfernt, durch einen Umbau zum Versteck, Refugium oder Kellergewölbe verloren gegangene Räume werden wieder hergestellt. Bist du sicher, dass du das willst ??");
...
else if ($_GET[act]=="rename")
{
    ...
    if (!$_POST[housename])
    {
        ...
        rawoutput("Gebe einen neuen Namen für dein Haus ein: <input name='housename' maxlength='40' value='$row[housename]'>");
    ...
    else
    {
        ...
        else
        {
        	...
        	output("`2Dein Haus `@$row[housename]`2 heißt jetzt `@".$fixed."`2.");
...
else if ($_GET[act]=="desc")
{
    ...
    else
    {
        ...
        output("`2Die Beschreibung wurde geändert.`n`0".$fixed."`2.");
...
WHERE (i.tpl_id="prive" OR i.tpl_id="privb") AND i.value1='.$session['housekey'].' AND i.owner='.$session['user']['acctid'];
    ...
    if ($session['user']['house']==$session['housekey'] && $row['status'] >= 10)
    {
        ...
        addnav('Privatgemächer vergeben','houses_private.php?op=raum_geben&private='.$session['user']['acctid'],false,false,false,false);
        addnav('Privatgemächer abnehmen','houses_private.php?op=raum_nehmen&private='.$session['user']['acctid'],false,false,false,false);
    ...
    if ($session[user][house]==$session[housekey])
    {
        ...
        addnav("Beschreibung ändern","inside_houses.php?act=desc",false,false,false,false);
    ...
    {
        addnav("K?Ins Kellergewölbe","housefeats.php?act=cry");
    }
    ...
    {
        addnav("r?Amüsieren (2000 Gold)","housefeats.php?act=amuse");
    }
    ...
    {
        addnav("q?Gefangene quälen","housefeats.php?act=torture");
    }
    ...
    {
        addnav("K?Mütterchens Kohlsuppe kosten","housefeats.php?act=soup");
    }
    ...
    {
        addnav("S?Zum Schmuckhändler","housefeats.php?act=gems");
    ...
    if (($row[status]>=60) && ($row[status]<70) && ($session['user']['race']=='vmp'))
    {
	    ...
	    if(($rowk['state']>0) && ($rowk['state']<19)) {
        	addnav("b?".$rowk['name']." beißen","housefeats.php?act=dbite");
	    }
    }
    if (($row[status]>=60) && ($row[status]<70) && ($session['user']['race']=='wwf'))
    {
	    ...
	    if(($rowk['state']>0) && ($rowk['state']<19)) {
        	addnav("b?".$rowk['name']." beißen","housefeats.php?act=dbite2");
	    }
    }
