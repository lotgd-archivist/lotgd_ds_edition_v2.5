			...
			while ($row = db_fetch_assoc($result)) {
				...
				output("
						<tr class='$bgclass'>
							<td>"
								.($row['activated'] == '0' 
									? create_lnk('Aktivieren','sulib.php?op=new_books&subop=activate&id='.$row['bookid']) 
									: create_lnk('Erneut Aktivieren','sulib.php?op=new_books&subop=activate_again&id='.$row['bookid'])
								)
								."|"
								.create_lnk('Löschen','sulib.php?op=del_post&id='.$row['bookid']),true);
...
case "edit_post":
	...
	if ($_GET['subop']=="button") {
		...
		elseif (isset($_POST['deactivate'])) {
			redirect("sulib.php?op=new_books&subop=deactivate&id=".$_GET['id']);
		}
	...
	if ($row['activated']=='0' || $row['activated'] == '2') {
		...
		addnav("Deaktivieren","sulib.php?op=new_books&subop=deactivate&id=".$_GET['id']);
	}
	if($row['recommended']) {
