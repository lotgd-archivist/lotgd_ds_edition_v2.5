...
switch($_GET[op])
	...
	case "magicroom":
		...
		switch(rand(0,10))
			{
			case 0:
				output("`@betritts den `4Raum des Todes`@. Überall wandeln Seelen von gefallenen Kriegern und Monstern herum und in der Mitte des Raums steht ein Altar mit einem Trank. Du gehts zu dem Trank und trinkst ihn aus. Dabei bemerkst wie er deine Seele säubert.`n`n");
				output("`4`bDu hast 5 Gefallen im Totenreich bekommen.`b`n`n");
				output("`@Der Raum fängt an sich zu drehen. Dir wird ganz schwindelig und als der Raum zum stehen kommt befindest du zurück dich im Wald. Du glaubst kaum, dass dies nur ein Traum war.");
			...
			case 1:
				output("`@betritts den `4Raum des Wissens`@. Überall stehen Regale mit Büchern herum. Du lässt dir ein wenig Zeit um dir ein paar Bücher durchzulesen.`n`n");
			...
			case 2:
				output("`@betritts den `4Raum der Schätze`@. Überall stehen Kisten mit Gold herum. Du denkst, dass es schon keiner merken wird, wenn du ein wenig Gold mitnimmst.`n`n");
			...
			case 9:
			case 10:
				output("`@betritts den `4Raum der unsterblichen Ruhe`@. Überall stehen Betten und Brunnen mit klarem kühlem Wasser herum. In der Mitte des Raumes steht ein prachtvoller Brunnen mit grün-blauem Wasser. Du bist erschöpft und trinkst von dem Wasser.`n`n");
	...
	case "away":
		switch(rand(0,10))
			...
			case 6: case 7:
				output("`@Du schaffts es deine Hand von dem Türgriff loszureissen und rennst so schnell du kannst.");
