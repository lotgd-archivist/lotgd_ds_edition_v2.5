...
switch($_GET[op]){
	...
	case "bartender":
		if ($_GET['what']==""){
			...
			output("<input name='testtext'></form>",true);
			...
			if ($_GET[who]==""){
				...
				if ($_GET['subop']!="search"){
					...
					$sql = "SELECT a.name,alive,location,sex,level,laston,loggedin,login,restatlocation FROM accounts a WHERE (locked=0 AND name LIKE '$search') ORDER BY level DESC";
					//output($sql);
					$result = db_query($sql) or die(db_error(LINK));
				...
				if ($session[user][gold]>=100){
					$sql = "SELECT 
									acctid,a.name,alive,location,maxhitpoints,gold,sex,level,weapon,armor,attack,defence,r.colname,charm 
							FROM accounts a
							LEFT JOIN races r ON r.id = a.race
							WHERE login=\"$_GET[who]\"";
					$result = db_query($sql) or die(db_error(LINK));
					if (db_num_rows($result)>0){
						...
						output("`4`bRasse:`b".$row['colname']."`n");
			...
			$sql="SELECT acctid,name,location,loggedin,laston,alive,housekey,activated,restatlocation FROM accounts WHERE acctid=".$_GET[who];
			$result = db_query($sql) or die(db_error(LINK));
			$row = db_fetch_assoc($result);
			...
			if ($row[location]==USER_LOC_PRISON) $loc="im Knascht";
			// end houses
