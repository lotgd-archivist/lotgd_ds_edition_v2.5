...
if ($_GET[op]=="")
	...
	addnav("D?Nimm den Dolch","forest.php?op=dagger");
	addnav("S?Nimm den Schädel","forest.php?op=skull");
	addnav("t?Nimm den Stab","forest.php?op=wand");
	addnav("R?Nimm das Rechenbrett","forest.php?op=abacus");
	addnav("B?Nimm das Buch","forest.php?op=book");
	addnav("K?Nimm den Kristallblitz","forest.php?op=bolt");
	...
	if (e_rand(0,1)==0)
		...
		{
		output("`#Du nimmst den Dolch von seinem Platz. Doch er löst sich in deinen Händen in Luft auf und du fühlst eine Welle von Energie in deinen Körper strömen!`n`n  `&Du erhältst 2 Level in Diebeskünsten!");
...
//	addnav("Zurück in den Wald","forest.php");
	...
	if (e_rand(0,1)==0)
		...
		{
		output("`#Du greifst nach dem Schädel. Vor deinen Augen löst sich der Schädel auf und du fühlst eine Energiewelle in deinen Körper fahren!`n`n  `&Du erhältst 2 Levels in Dunklen Künsten!");
...
//	addnav("Zurück in den Wald","forest.php");
	...
	if (e_rand(0,1)==0)
		...
		{
		output("`#Du hebst den Stab von seinem Platz auf. In einem Lichtblitz verschwindet er und eine seltsame Kraft durchströmt deinen Körper!`n`n  `&Du erhältst 2 Levels in Mystischen Kräften!");
		$session['user']['specialtyuses'][magic]+=2;
...
//		addnav("Zurück in den Wald","forest.php");
	...
	elseif($bchance==2)
		{
		output("`#Du greifst nach dem Kristallblitz. Als du den Blitz gerade berührst, wirst du rückwärts auf den Boden geschleudert. Du kommst schnell wieder auf die Beine und fühlst dich sehr mächtig!`n`nDu steigst in jeder Fertigkeit 2 Level auf!");
		$session[user][specialtyuses][thievery]+=2;
		$session[user][specialtyuses][darkarts]+=2;
		$session[user][specialtyuses][magic]+=2;
		$session[user][specialtyuses][thieveryuses]++;
