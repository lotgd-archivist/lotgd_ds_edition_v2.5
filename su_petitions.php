...
switch($str_op) {
	...
	case 'setfor':
		...
		if(isset($_POST['p_for'])) {
			$sql = 'UPDATE petitions SET p_for="'.addslashes(stripslashes($_POST['p_for'])).'" WHERE petitionid='.$pid;
			db_query($sql);
		}
	...
	case 'view':
		...
		if (!empty($row['login'])) {
			...
			output('<a href="#" onClick="'.popup($str_maillnk).';return false;">
		...
		// Superuser
		$res = db_query('SELECT login FROM accounts WHERE superuser>0 ORDER BY login ASC');
		
		$str_lnk = $str_filename.'?op=setfor&pid='.$int_pid;
		addnav('',$str_lnk);
		$str_out = '`n<form method="POST" action="'.$str_lnk.'">
					Zu bearbeiten von: <select name="p_for" onchange="this.form.submit();">
					<option value="">Allen</option>
					<option> ~~~ </option>';
		foreach ($arr_grps as $g) {
			$n = strip_appoencode($g[1],3);		
			$str_out .= '<option value="'.$n.'" '.($row['p_for'] == $n ? 'selected="selected"' : '').'>'.$n.'</option>';
		}
		$str_out .= '<option> ~~~ </option>';
		while($a = db_fetch_assoc($res)) {
			$str_out .= '<option value="'.$a['login'].'" '.($row['p_for'] == $a['login'] ? 'selected="selected"' : '').'>'.$a['login'].'</option>';
		}
		$str_out .= '</select>
	...
	default:
		...
		while( $row = db_fetch_assoc($result) ){
			...
			$str_out .= '	</td>
							...
							<td>'.$ARR_PETITION_KATS[$row['kat']].(!empty($row['p_for']) ? '`nFür: '.($session['user']['login'] == $row['p_for'] ? '`$':'').$row['p_for'].'`0' : '').'</td>
						</tr>';
