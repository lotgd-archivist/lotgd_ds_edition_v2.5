...
WHERE ";
    ...
    if (db_num_rows($result)>0)
    {
        ...
        else if (($row['pvpflag'] > $pvptimeout) && (($session['user']['profession']==0) || ($session['user']['profession']>2)))
        {
        ...
        else if ((($session['user']['dragonkills'] > $row['dragonkills']+5) && ($row['location']!=USER_LOC_HOUSE)) && (($session['user']['profession']==0) || ($session['user']['profession']>2)))
        {
