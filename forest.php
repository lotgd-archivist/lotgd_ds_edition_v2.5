...
{
    $sql = 'SELECT name,state FROM disciples WHERE state=15 AND master='.$session['user']['acctid'];
	$result = db_query($sql) or die(db_error(LINK));
	if (db_num_rows($result)>0)
	{
    $rowk = db_fetch_assoc($result);
    output($rowk['name']."`& schnappt sich deinen Beutel mit ".$session['user']['gold']." und bringt ihn eiligst zur Bank.`nDu geniesst eine Weile die frische Waldluft und es dauert gar nicht lange bis dein Knappe wieder zurück ist.");
...
if ($_GET['op']=='search')
{
	...
	else
	{
		...
		if ($specialtychance==0 || isset($_GET['forest_special'])){
			...
			$waldspecial = mysql_result(db_query('SELECT filename FROM waldspecial WHERE prio <= '.e_rand(0,3).' AND dk <='.$session['user']['dragonkills'].' AND released=1 ORDER BY RAND() LIMIT 1'),0,'filename');
