...
else if ($_GET[act]=="soup")
{
    if ($session['user']['turns']>0)
    {
        ...
        output("`5Mein Geheimrezept beinhaltet einen ganz besonderen und seltenen magischen Pilz. Daher muss ich von dir pro Teller `^100 Goldmünzen`5 verlangen.`nAuch gebe ich zu, dass meine Sehkraft nicht mehr die beste ist, und dass ich mich beim Pilzepflücken ab und an mal leicht vertue...`n");
...
else if ($_GET[act]=="disciple2")
{
    ...
    if (db_num_rows($result)==0)
    {
        ...
        $sql = "INSERT INTO disciples (name,state,level,master) VALUES ('".addslashes($name)."',$state,'0','".$session['user']['acctid']."')";
        db_query($sql) or die(sql_error($sql));
    }
    else
    {
        $sql = "UPDATE disciples SET name='".addslashes($name)."',state=$state,level='0',best_one='0' WHERE master = ".$session['user']['acctid'];
        db_query($sql) or die(sql_error($sql));
    }
...
else if ($_GET[act]=="beater2")
{
    If (($session['user']['gold']>=2000) && ($session['user']['gems']>=2))
    {
        ...
        {
            $sql = "SELECT accounts.acctid,name,hitpoints,login,loggedin,account_extra_info.beatenup,account_extra_info.timesbeaten FROM accounts LEFT JOIN account_extra_info ON account_extra_info.acctid=accounts.acctid WHERE accounts.acctid='$_GET[who]'";
            $result = db_query($sql) or die(db_error(LINK));
            $row = db_fetch_assoc($result);
...
else if ($_GET[act]=="dbite2")
{
    ...
    unset($session['bufflist']['decbuff']);
    $sql = "UPDATE disciples SET state=19 WHERE master = ".$session['user']['acctid'];
    db_query($sql) or die(sql_error($sql));
    addnav("Zurück","inside_houses.php");
    ...
    if ($_GET[who]=="")
    {
        ...
        else
        {
			...
			if(strlen($_POST['name']) < 3) {
				redirect('housefeats.php?act=sendtrophy2&id='.$_GET['id']);
			}

			$search = str_create_search_string($_POST['name']);

            $sql = "SELECT name,alive,loggedin,login FROM accounts WHERE (name LIKE '".addslashes($search)."') ORDER BY name ASC LIMIT 100";
            //output($sql);
            $result = db_query($sql) or die(db_error(LINK));
            $max = db_num_rows($result);
...
else if ($_GET[act]=="sendtrophy3")
{
    ...
    $sql = "SELECT acctid,name,login,pvpflag FROM accounts WHERE acctid=".$userid;
    $result = db_query($sql) or die(db_error(LINK));
    $row = db_fetch_assoc($result);
    ...
    // Hot Items
	if($item['hot_item'] == 2 && PVP_IMMU == $row['pvpflag']) {
		output('`3Nein, wie fies! '.$row['name'].' schickt dir das Stück postwendend wieder zurück, weil er seinen Schutz
	...
	if(!$item_hook_info['hookstop']) {
		...
		$item['gold'] = round($item['gold'] * 0.45);
		$item['gems'] = round($item['gems'] * 0.45);
		$item['gold'] = min($item['gold'],2500);
		$item['gems'] = min($item['gems'],1);
...
else if ($_GET[act]=="searchdisciple")
{
    ...
    output("`&Du näherst dich den Abenteurern, die in einem kleinen Saal im Keller umgeben von allen möglichen Karten und viel Plunder ihre Heldentaten zum Besten geben.`n
    Dir kommt in den Sinn, dass diese Männer vielleicht wissen könnten wohin man deinen armen Knappen verschleppt hat.`n
    Du beginnst ein Gespräch und sehr schnell wird deutlich, dass es dich einiges kosten wird, Informationen zu erlangen.`n
    Die Männer wollen `^".$cost."`& Goldstücke sehen, bevor sie auch nur einen Ton von sich geben.`n
    Sie weisen dich aber vorher darauf hin, dass dies ein schwieriges Unterfangen sein kann. Du solltest dich gut darauf vorbereiten, da es kein Zurück mehr gibt!`n
    Was tust du ?");
