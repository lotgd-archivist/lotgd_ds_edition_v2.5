	...
	else if ($_GET[op]=="build")
	{
		if ($_GET[act]=="start")
		{
			...
			else
			{
                ...
                $sqlfree = "SELECT * FROM houses WHERE status=6 ORDER BY houseid DESC";
...
if ($number_of_houses>0 && $rowcheck['status']>0)
			{
				output("`@Du hast bereits Zugang zu einem fertigen Haus und brauchst kein weiteres. Wenn du ein neues oder ein eigenes Haus bauen willst, musst du erst aus deinem jetzigen Zuhause ausziehen.");
			}
			...
			{
				output("`@Du besichtigst die Baustelle deines neuen Hauses mit der Hausnummer `3$rowcheck[houseid]`@.`n`n");
				$goldtopay=$goldcost-$rowcheck[gold];
				$gemstopay=$gemcost-$rowcheck[gems];
				$done=round(100-((100*$goldtopay/$goldcost)+(100*$gemstopay/$gemcost))/2);
...
output("</table>",true);
		...
		else
		{
			...
			{
				output("`@Glückwunsch zu deinem neuen Haus!`n`n(Falls in deinem Haus nicht alle Schlüssel vorhanden sind, schreibe bitte eine Anfrage. Fehlende und verlorene Exemplare werden dir ersetzt)");
	...
	else if ($_GET[op]=="searchtype")
	{
		...
		if (!$_REQUEST[what])
		{
			output("`@Vor einem unscheinbaren Haus entdeckst du eine Tafel die davon kündet daß man hier eine Übersicht über alle Häuser im Dorf erhalten kann. Neugierig trittst du ein.");
			output("`n`nEin junger Elf in grüner Dienstkleidung spricht dich an: `^Seid gegrüßt ".($session['user']['sex']?"Verehrte Dame":"Ehrenwerter Herr")."! Welche Art der Dienstleistung Ihr auch suchet, in unseren `GGrünen Seiten`^ sind alle Anbieter des Dorfes eingetragen.");
