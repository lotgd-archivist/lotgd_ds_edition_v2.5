...
switch ($_GET['op'])
{
	...
	case 'read':
	{
		...
		if (db_num_rows($result)>0)
		{
			...
			`b`2Gesendet:`b `^'.date(MAIL_DATE_FORMAT,strtotime($row["sent"])).'`n
	...
	case 'write':
	{
		...
		$str_subtitle = 'Brieftaube verfassen';
	...
	case 'address':
	{
		$str_subtitle = 'Brieftaube senden';
		$output.='
		<form action="mail.php?op=write" method="POST">
		`2<u>A</u>n: <input name="to" accesskey="a"> <input type="submit" class="button" value="Suchen!"></form>';
	}
	break;
	case 'buch':
	{
		...
		**********************************************/
		...
		$sql = 'SELECT y.row_id,y.player,y.descr,a.login,a.name,a.loggedin,a.laston,a.activated,('.user_get_online().') AS online 
				FROM yom_adressbuch y LEFT JOIN accounts a ON a.acctid=y.player WHERE y.acctid='.$session['user']['acctid'].' ORDER BY online DESC, login ASC;';
		$result = db_query($sql);
		...
		else
		{
			...
			for ($i=0;$i<$menge;$i++)
			{
				...
				<tr>
                   ...
                   <td> '.$k['descr'].'</td>
				   <td>&nbsp;&nbsp;</td>
				   <td>'.(($k['online'])?'`@online':'`4offline').'</td>
				   <td>&nbsp;&nbsp;</td>
	...
	case 'neuerkontakt':
	{
		...
		`2<u>N</u>ame: <input name="to" accesskey="N" value="'.$_GET['name'].'"> <input type="submit" class="button" value="Kontakt suchen"></form>';	
	}
	break;
...
if(getsetting('show_yom_contacts',1)==1 || $session['user']['superuser']>0)
{
	$main_output .= '
		<td>
			<script language="javascript">window.resizeTo(800,450);</script>
...
$main_output.='
	...
	<tr><td colspan="4" align="center">`b`&'.$str_subtitle.'`b`n
	~~~</td></tr>
</table>';
