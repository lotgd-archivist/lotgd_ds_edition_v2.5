...
switch($str_op) {
	...
	case 'search':	
		...
		$bool_lockbio = su_check(SU_RIGHT_LOCKBIOS);
		...
		while($l = db_fetch_assoc($res)) {
			...
			$str_out .= '<tr class="trlight">
							<td><a name="#'.$l['acctid'].'"></a>`bID '.$l['acctid'].': '.$l['name'].
								...
								.($bool_lockbio ? ' - '.create_lnk('Bio '.($l['biotime'] == BIO_LOCKED ? 'entsperren' : 'sperren'),$str_filename.'?op=lock_bio&uid='.$l['acctid'].'&ret='.urlencode($str_baselnk.$page),true,false,($l['biotime'] == BIO_LOCKED ? '' : 'Bio wirklich sperren und löschen? (Sicherung davor nicht vergessen!)')) : '')
							.'</td>
						</tr>
						<tr>
							<td>`n';
