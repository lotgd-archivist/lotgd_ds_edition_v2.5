...
//pack browser array
// values [0]= user agent identifier, lowercase, [1] = dom browser, [2] = shorthand for browser, [3]=long name
$a_browser_types[] = array('opera', true, 'op', 'Opera' );
$a_browser_types[] = array('msie', true, 'ie', 'Internet Eplorer' );
$a_browser_types[] = array('konqueror', true, 'konq', 'Konqueror' );
$a_browser_types[] = array('safari', true, 'saf', 'Safari' );
$a_browser_types[] = array('gecko', true, 'moz', 'Mozilla' );
$a_browser_types[] = array('mozilla/4', false, 'ns4', 'Netscape' );
...
function browser_set( $acctid = 0, $browserinfo = false ){
	...
	if( $browserinfo === false ){
		$browserifo = browser_detection( 'full' ); 
	}
	
	db_query( 'UPDATE account_stats SET browser="'.$browserifo[0].'", browser_version="'.$browserifo[1].'" WHERE acctid='.$acctid );
}
