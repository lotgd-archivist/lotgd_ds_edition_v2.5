<?php
/**
 * commentary.lib.php: Funktionen zur Bereitstellung von Chatbereichen
 * @author LOGD-Core / Drachenserver-Team
 * @version DS-E V/2
*/

$rcomment_sections = array('village'=>1,'marketplace'=>1,'garden'=>1,'expedition_wastes'=>1,'prison'=>1,'pool'=>1,'fishing'=>1);
$rcomment_sections_inside = array('prison'=>1);
$rcomment_sections_public = array('village'=>1,'marketplace'=>1,'garden'=>1,'expedition_wastes'=>1,'prison'=>1,'pool'=>1,'fishing'=>1);
...
/**
 * Fügt in $_POST vorhandene Kommentare in die passende Sektion ein
 *
 * @param bool Säufereffekte auf Geschriebenes (optional, Standard TRUE)
 * @return bool TRUE, wenn ein Kommentar hinzugefügt wurde, FALSE, wenn nicht
 */
function addcommentary($alc=true)
{
	...
	trim($_POST['insertcommentary'][$section])!="") 
	{
		...
		if ($row['comment']!=stripslashes($commentary) || $row['author']!=$session['user']['acctid'])
		{
			...
			$commentary = addslashes(stripslashes($commentary));
...
/**
 * Zeigt Chatsektion an.
 *
 * @param string Interner Bezeichner der Sektion
 * @param string Nachricht vor Chatfeld (optional)
 * @param int Beiträge pro Seite (optional, Standard 10)
 * @param string Tätigkeitsverb (optional, Standard 'sagt')
 * @param bool Datum vor Posts (optional, Standard FALSE)
 * @param bool Eingabebereich anzeigen (optional, Standard TRUE)
 * @param bool Ohne Funktion
 * @param bool Verlängerte Posts erlauben (optional, Standard FALSE)
 * @param bool Button zum Ausblenden von Systemposts (optional, Standard FALSE)
 * @param bool Link für Bios (optional, Standard TRUE)
 * @param int Wenn > 1, können Kommentare nicht von allen Superusern gelesen werden (optional, Standard 1)
 */
function viewcommentary($section,
	...
	if(time() - $session['modcalled'] >= 300 && getsetting('enable_modcall',0) && $session['user']['loggedin']) {
		$req = 'comment_funcs.php?callmod=1&section='.$section.'&ret='.urlencode($REQUEST_URI);
		addnav('',$req);
		output("&nbsp;&nbsp;&nbsp;&nbsp;`&[<a href=\"$req\" onClick='return confirm(\"Wirklich Mod rufen?\");'>Spam melden</a>]",true);
	}
...
/**
 * Fügt Kommentar zu Chatsektion hinzu.
 *
 * @param int AcctID des Autors
 * @param string Kommentar
 * @param string Chatsektion
 * @param int Wenn > 1, können Kommentare nicht von allen Superusern gelesen werden (optional, Standard 1)
 * @param int Wenn 0, ist Kommentar ein Systempost (optional, Standard 0)
 * @author talion
 */
function insertcommentary ($author,$msg,$section,$su_min=1,$self=0) {
	...
	db_insert	(
				'commentary',
				array( 'postdate'=>array('sql'=>true,'value'=>'NOW()'), 'author'=>$author, 'comment'=>$msg, 'section'=>$section, 'su_min'=>$su_min, 'self'=>$self)
				);
