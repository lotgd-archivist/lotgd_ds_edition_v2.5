...
by Alucard 
*/
$GLOBALS['ci_conn_host'] 	= "";			//Host
$GLOBALS['ci_conn_user'] 	= "";		//user
$GLOBALS['ci_conn_pass']	= "";	//password
$GLOBALS['ci_conn_db']		= "";		//datenbankname

$GLOBALS['ci_usertable'] 	= "smf_members"; 		//tabelle für user
$GLOBALS['ci_row_id'] 		= "ID_MEMBER";			//spaltenname-id
$GLOBALS['ci_row_name'] 	= "memberName";			//spaltenname-username
$GLOBALS['ci_row_real_name']= "realName";			//Realer Name des Nutzers (Optional)
$GLOBALS['ci_row_pass'] 	= "passwd";				//spaltenname-password
$GLOBALS['ci_row_mail'] 	= "emailAddress";		//spaltenname-emailadresse
$GLOBALS['ci_row_active'] 	= "is_activated";		//spaltenname-user ist aktiv
$GLOBALS['ci_row_reg'] 		= "dateRegistered";		//spaltenname-registrations datum

global $ci_conn_host, $ci_conn_user, $ci_conn_pass, $ci_conn_db, $ci_usertable, $ci_row_id, $ci_row_name, $ci_row_real_name, $ci_row_pass, $ci_row_mail, $ci_row_active, $ci_row_reg;
...
$GLOBALS['ci_sql'] = array( 	'getnextid' => "SELECT ".$ci_row_id." AS startid FROM ".$ci_usertable." ORDER BY ".$ci_row_id." DESC LIMIT 1",
					'insert' 	=> "INSERT INTO ".$ci_usertable." (".$ci_row_id.",".$ci_row_name.",".$ci_row_real_name.",".$ci_row_pass.",".$ci_row_mail.",".$ci_row_reg.",".$ci_row_active.") VALUES (%d, '%s', '%s', '%s', %d, %d, 1)",
					'ban'		=> "UPDATE ".$ci_usertable." SET ".$ci_row_active."=%d WHERE ".$ci_row_id."=%d LIMIT 1",
					'delete'	=> "DELETE FROM ".$ci_usertable." WHERE ".$ci_row_id."=%d LIMIT 1",
					'rename' 	=> "UPDATE ".$ci_usertable." SET ".$ci_row_name."='%s', ".$ci_row_real_name."='%s' WHERE ".$ci_row_id."=%d LIMIT 1",
					'changepw'	=> "UPDATE ".$ci_usertable." SET ".$ci_row_pass."='%s' WHERE ".$ci_row_id."=%d LIMIT 1",
					'checkname' => "SELECT ".$ci_row_id." FROM ".$ci_usertable." WHERE LOWER(".$ci_row_name.")='%s' LIMIT 1"
				);
...
function ci_importusers($aUser, $setactive=1)  
{
	...
	for($i=0;$i<$count; ++$i){
		...
		$qry = sprintf($ci_sql['insert'], ++$start, addslashes($aUser[ $i ]['name']),addslashes($aUser[ $i ]['name']), addslashes($aUser[ $i ]['pass']), $aUser[ $i ]['mail'], time(), $setactive);
		if( !db_query($qry) ){
			$aUser[ $i ]['id'] = 0;
			//$ret = db_error($G_CI_CONN);
		}
		else{
			$aUser[ $i ]['ci_id'] = $start;
		}
...
function ci_rename($id, $name)
{
	...
	db_query(sprintf($ci_sql['rename'], addslashes(strtolower($name)),addslashes(strtolower($name)), $id)) or die (db_error($G_CI_CONN));
