...
define('SU_RIGHT_RP',48);
define('SU_RIGHT_MULTI',49);
define('SU_RIGHT_EDITOREQUIPMENT',50);
define('SU_RIGHT_BIOS',51);
define('SU_RIGHT_LOCKBIOS',52);
define('SU_RIGHT_EDITORRACES',53);

// Namen / Beschreibungen für SU-Rechte
$ARR_SURIGHTS = array(   'support' => 'Support / PR'
						,SU_RIGHT_PETITION => array('desc'=>'Darf Anfragen bearbeiten?')
						...
						,SU_RIGHT_EDITORLIBRARY => array('desc'=>'Darf Bibliothek bearbeiten?')
						...
						,SU_RIGHT_EDITORWORLD => array('desc'=>'Darf Welt-Editoren benutzen (Monster;Spott;Rätsel;Runen;)?')
						,SU_RIGHT_EDITOREQUIPMENT => array('desc'=>'Darf Waffen + Rüstungen bearbeiten?')
						,SU_RIGHT_EDITORMOUNTS => array('desc'=>'Darf Stalltiere bearbeiten?')
						,SU_RIGHT_EDITORRACES => array('desc'=>'Darf Rassen bearbeiten?')
						...
						,SU_RIGHT_DONATIONS => array('desc'=>'Darf DP vergeben / abnehmen?')
						...
						,SU_RIGHT_BIOS => array('desc'=>'Darf Bios + Avas überwachen?')
						,SU_RIGHT_STEALTH => array('desc'=>'Darf Stealthmodus nutzen?')
						,SU_RIGHT_NEWS => array('desc'=>'Darf News + Aufzeichnungen eintragen / löschen?')
						,SU_RIGHT_MULTI => array('desc'=>'Darf Multis kontrollieren?')
						,'deep' => 'Gefährliches'
						...
						,SU_RIGHT_RP => array('desc'=>'Spielleiterfunktionen (/msg)?')
						...
						,SU_RIGHT_LOCKBIOS => array('desc'=>'Darf Bios + Avas sperren?')
						,'Sonstiges'=>'Sonstiges'
						,SU_RIGHT_WARTUNG => array('desc'=>'Übergeht Wartungsmodus?')
						,SU_RIGHT_DEBUG => array('desc'=>'Debug-Funktionen (Male testen Gericht / Stadtwache / Tempel betreten etc.)?')
						);
 ...
 */
function su_check_other ($int_right) {
...
function kill_cheater () {
	...
	systemlog(' - HACKVERSUCH -`n
				...
				Referer: '.$_SERVER['HTTP_REFERER'],$session['user']['acctid']);
...
function delban($int_banid)
{
	...
	// User ermitteln, die der Ban betreffen könnte
	$sql = 'SELECT a.acctid,a.login,incommunity FROM accounts a LEFT JOIN account_extra_info USING(acctid) WHERE
				'.(!empty($str_ip) ? 'lastip = "'.addslashes($str_ip).'" OR ':'').'
				'.(!empty($str_id) ? 'uniqueid = "'.addslashes($str_id).'" OR ':'').'
				'.(!empty($str_mail) ? 'emailaddress = "'.addslashes($str_mail).'" OR ':'').'
				'.(!empty($str_login) ? 'login = "'.addslashes($str_login).'" OR ':'').'
