...
function js_preview ($str_fieldname) {
    ...
    $str_out = appoencode('`0').'<span id="'.$str_fieldname.'_prev"></span>
				...
				window.setTimeout("init_prev_'.$str_fieldname.'()",500);

				function init_prev_'.$str_fieldname.' () {

    				var func = function prev_tmp_'.$str_fieldname.' (e) { var obj = document.getElementsByName("'.$str_fieldname.'")[0];input_prev(obj);obj.focus(); }
    				var obj = document.getElementsByName("'.$str_fieldname.'")[0];
    				if (obj.addEventListener) {
                        obj.addEventListener("keyup", func, false);
                    }
                    else if (obj.attachEvent) {
                        obj.attachEvent("onkeyup", func);
                    }
                    input_prev(obj);
                }
...
/**
 * @desc Macht eine print_r-Ausgabe auf dem Bildschirm wenn der User ein Superuser ist
 *
 * @param array Auszugebender Array
 */
function admin_print_r($arr_out)
{
	global $session;
	if($session['user']['superuser']>=1)
	{
...
* @param int		Wartezeit, die der Spieler auf aktueller Seite verbringen muss, ehe die Playlist aktualisiert wird.
* @return -
*/
function music_set ($str_cat, $int_secs2wait = 20) {
	...
	$arr_debug = array(
						...
						,'inn' => 'schenke'
						,'waldsee' => 'waldsee'
						,'poseidonpool' => 'poseidonpool'
						,'forest' => 'Wald'
						,'shades' => 'Unterwelt'
						,'garden' => 'Garten'
						,'boss' => 'Drachenkill'
						,'abandoned_castle' => 'verlassenes_schloss'
						,'Schaukel' => 'schaukel'
						);
	...
	rawoutput('<script>var player_active_time=0;var player_start_time='.$int_secs2wait.';var current_pl="'.$m.'";window.setInterval("player_active_time+=1",1000);</script>');
...
function get_appoencode ($bool_forcereload=false) {
	...
	$arr_appoencode = cache_get('appoencode');
	...
	if(false === $arr_appoencode || $bool_forcereload) {
		...
		if(db_num_rows($res)) {
			...
			$arr_appoencode = array();

			while($c = db_fetch_assoc($res)) {

				$arr_appoencode[$c['code']] = $c;

			}

			cache_set('appoencode',$arr_appoencode);
...
* @author talion
*/
function strip_appoencode ($str_input,$int_mode=1) {
 ...
 * @return string Komprimierter Text
 */
...
function fightnav($allowspecial=true, $allowflee=true){
	...
	if ($allowspecial) {
		...
		$arr_specialties = cache_get('specialties');
		...
		if(false === $arr_specialties) {
			...
			cache_set('specialties',$arr_specialties);
		}
		...
		$result = item_list_get( ' owner='.$session['user']['acctid'].' AND value1>0 AND (deposit1=0 OR deposit1='.ITEM_LOC_EQUIPPED.')
				AND (battle_mode = 1 OR battle_mode = 3) ', ' GROUP BY name ORDER BY value1 ASC, name ASC, id ASC', true , ' SUM(value1) AS anzahl, name, id ' );
...
function charstats()
{
	...
	if ($session['loggedin']){
		...
		$player = (1 ? '<a href="#" target="_blank" onClick="'.popup('music.php').';return false;">Wanderbarde</a>' : '');
		...
		.(!empty($player) ? templatereplace('statrow',array('title'=>'Wanderbarde','value'=>$player)) : '')
...
/**
 * Erzeugt aus Formularschablone und Formulardatensatz fertigen Formularcode
 *
	...
	foreach($layout as $key=>$val)
	{
			...
			|| $countt == 1)	// Bei jedem Titelfeld ODER wenn noch kein Titelfeld geöffnet wurde
		{
			...
			if(!$info[2] || $countt == 1) {
				...
				if( $countt > 1 && ($countt-1) % 4 == 0) {	// jeden 4. Reiter
		...
		switch($info[1])
		{
			...
			break;
			case 'text':
				$table.="<input name='$key' value=\"".HTMLEntities($row[$key])."\" size='30' maxlength=\"$info[2]\"'>";
			break;
			// added by talion
			case 'preview':
...
/**
 * @desc Läd ein vorgegebenes Template
 *
...
function loadtemplate($templatename)
{
	...
	{
		$templatename='dragonslayer_1.html';
	}
...
function maillink()
{
	...
	if($row['seencount']>=getsetting('inboxlimit',50) && $session['user']['superuser']<1)
	{
		...
		<div style="z-index:5; float:left; padding-bottom:10px; padding-top:10px; background-color:black; vertical-align:middle; width:100%; height:auto; font-size:20px; top:2px; left:0px; position:relative; border:2px solid red; background-image: url(templates/dragonslayer_1/bg_tile_stressedmetal.jpg);">
			...
			<a href="#" target="_blank" onClick="'.popup('mail.php').';return false;" >Brief-Schatulle aufräumen</a>
...
function page_header($title=GAME_VERSION)
{
	...
	if(getsetting('rss_enable_motd_feed',false) == true)
	{
		$str_rss = '<link rel="alternate" type="application/rss+xml"
					title="RSS" href="'.getsetting('rss_file_rel_path','').'" />';
	}
	if($session['user']['lastmotd'] == '0000-00-00 00:00:00')
	{
		$header=str_replace('{headscript}',$str_colors_css.$str_rss.'<script language="JavaScript" type="text/javascript">'.popup('motd.php').'</script>',$header);
		$session['needtoviewmotd']=true;
	}
	else
	{
		$header=str_replace('{headscript}',$str_colors_css.$str_rss,$header);
...
function popup($page,$width=550,$height=300)
{
	return "window.open('$page','".preg_replace("([^[:alnum:]])","",$page)."','scrollbars=yes,resizable=yes,width=$width,height=$height')";
}
...
function page_footer()
{
	...
	if ($paysite != '') {
		...
		<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
			...
			<input type="hidden" name="item_name" value="Spende aus '.getsetting('townname','Atrahor').' von '.preg_replace("/[`]./","",$session['user']['name']).'">
			<input type="hidden" name="item_number" value="'.htmlentities($session['user']['login']).":".$_SERVER['HTTP_HOST']."/".$_SERVER['REQUEST_URI'].'">
			<input type="hidden" name="no_shipping" value="1">
			<input type="hidden" name="cn" value="Your Character Name">
			<input type="hidden" name="cs" value="1">
			<input type="hidden" name="currency_code" value="USD">
			<input type="hidden" name="tax" value="0">
			<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Spenden!">
		</form>';
	}
	...
	$header=str_replace('{petition}','<a href="petition.php" onClick="'.popup('petition.php').';return false;" target="_blank" class="motd">Anfrage schreiben</a>',$header);
	...
	if($session['user']['superuser'] > 0) {
		...
		$help_link = 'su_help.php';
		$str_append .= '<tr><td align="right"><a href="#" target="_blank" onClick="'.popup($help_link).';return false;"><b>Hilfe</b></a></td></tr>';
	...
	{
		$footer=str_replace('{source}','<a href="source.php?op=show&file='.urlencode(preg_replace('/[?].*/','',($_SERVER['REQUEST_URI']))).'" target="_blank">Source</a>',$footer);
		$header=str_replace('{source}','<a href="source.php?op=show&file='.urlencode(preg_replace('/[?].*/','',($_SERVER['REQUEST_URI']))).'" target="_blank">Source</a>',$header);
	}
	else
	{
...
{
	global $output,$nestedtags,$header,$nav,$session,$logd_version;
	foreach($nestedtags as $key=>$val)
	{
		$output.='</'.$key.'>';
		unset($nestedtags[$key]);
	...
	$output.='</td></tr>
	           <tr><td bgcolor="#330000" align="center">Copyright 2002, Eric Stevens</td></tr>
	           <tr><td bgcolor="#000000" align="center">'.$logd_version.'</td></tr>
	           </table></body></html>';
