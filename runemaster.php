...
switch( $_GET['op'] ){
	...
	case 'master':
		...
		if( $session['user']['turns'] > 0 && item_count('tpl_id="r_raidho" AND owner='.$session['user']['acctid']) > 0  ){
			addnav('Zur Burg (Raidho)','runemaster.php?op=castle');
			addnav('','runemaster.php?op=castle');
			$out .= '<a href="runemaster.php?op=castle">Erkläre mir den Weg zur Orkburg für eine Raidho - Rune!</a>`n';
		}

		if( item_count('tpl_id="r_dagaz" AND owner='.$session['user']['acctid']) > 0 ){
			addnav('Entzaubern (Dagaz)','runemaster.php?op=unban');
		...
		if( item_count('tpl_id="r_jera" AND owner='.$session['user']['acctid']) > 0 ){
			addnav('Runden auffüllen (Jera)','runemaster.php?op=fillup');
		...
		if( item_count('tpl_id="r_ansuz" AND owner='.$session['user']['acctid']) > 0 ){
			addnav('Rüstung Gravieren (Ansuz)','runemaster.php?op=armorrename');
			addnav('','runemaster.php?op=armorrename');
			$out .= '<a href="runemaster.php?op=armorrename">Graviere meine Rüstung für eine Ansuz - Rune!</a>`n';
		}

		if( item_count('tpl_id="r_sowilo" AND owner='.$session['user']['acctid']) > 0 ){
			addnav('Knappen taufen (Sowilo)','runemaster.php?op=disciplerename');
			addnav('','runemaster.php?op=disciplerename');
			$out .= '<a href="runemaster.php?op=disciplerename">Taufe meinen Knappen für eine Sowilo - Rune!</a>`n';
		}
	...
	case 'pricelist':
		$out .= '`4`bPreisliste des Runenmeisters`b`n`n';
		$out .= '`&Knappen taufen: `qSowilo`n';
		$out .= '`&Zur Burg Reiten: `qRaidho`n';
		$out .= '`&Rüstung gravieren: `qAnsuz`n';
		$out .= '`&Runden auffüllen: `qJera`n';
		$out .= '`&Entzaubern: `qDagaz`n';
...
//Rüstung Gravieren
	case 'armorrename':
		...
		$out .= 'Für eine Ansuz - Rune kann ich Euch Eure Rüstung mit einer Gravur nach Wunsch prachtvoll verzieren!`n`n`n';
...
//Knappen taufen
	case 'disciplerename':
		$out .= 'Du fragst den Runenmeister, ob er deinen Knappen nach uraltem Brauch taufen könne. Er nickt und spricht zu dir:`&`n';
		$out .= 'Gewiss! Nur muss er etwas seiner Kraft opfern und ich brauche eine Sowilo Rune für dieses Ritual!`n`n';
		$out .= '`n`n`4`bTauft meinen Knappen auf:`b`0`n';
		$out .= js_preview('newname').'`n';
		$out .= '<form action="runemaster.php?op=disciplerename2" method="POST"><input name="newname" value="" size="30" maxlength="30"> <input type="submit" value="Taufen"></form>';
	...
	case 'disciplerename2':
		...
		$str_valid = user_rename(0, stripslashes(strip_appoencode($_POST['newname'],3)), false, false, USER_NAME_BADWORD | USER_NAME_BLACKLIST);
		if(true !== $str_valid) {
			$out .= '`^Als du dem Runenmeister den Namen nennst, schaut er dich an und sagt:`n`&';
			$out .= 'Dieser Name ist von den Göttern nicht erwünscht! Und ich glaube kaum, dass dein Knappe gern '.$str_name.'`& heissen will!';
			addnav('Schade!');
			...
			$out .= 'Du nennst dem Runenmeister den Namen und gibst ihm eine Sowilo-Rune. Er winkt deinen Knappen zu sich herran und geht mit ihm hinter einen Hügel, als du ihnen folgen willst, erblickst du niemanden und setzt dich nieder.`n';
			$out .= 'Nach einiger Zeit kommt dein Knappe und spricht zu dir:`n`&';
			$out .= 'Herr'.($session['user']['sex']?'in':'').'! Fortan sollt Ihr mich `#'.$str_name.'`& rufen, sobald ihr meine Dienste braucht.`n';
			$out .= '`^Darauf hin fügt der Runenmeister ergänzend hinzu:`n`&';
			$out .= 'Er ist nun leider etwas schwächer, aber ich bin überzeugt, dass er dieses Defizit, dank des Trainings mit Euch, schnell wieder wett gemacht haben wird!';
			addnav('Wie schön!');
			addnav('Freude!','runemaster.php?op=master');
			addnews($session['user']['name'].'`@\'s Knappe ist nun bekannt als '.$str_name);
			db_query('UPDATE disciples SET name="'.addslashes($str_name).'", level = IF(level>0,level-1,0) WHERE master='.$session['user']['acctid']);
