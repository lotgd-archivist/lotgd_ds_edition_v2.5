...
if($_GET['op'] != 'einladungen') {
	...
	if(empty($session['housekey']) || !empty($_GET['housekey'])) {
	...
	if($_GET['invited']) {
		...
		$session['housekey'] = $_GET['housekey'];
...
switch($op) {
	...
	case '':	// Chat anzeigen
        ...
        for ($i=1;$i<=$count;$i++){
			...
			if($item['furniture_private_hook'] != '' && $bool_not_invited && !$hooks[$item['furniture_private_hook']]) {
...
//			for($i=0;$i<sizeof($furniturenav);$i++){
	...
	case 'einladungen':
		...
		output('`@Du durchwühlst deinen Beutel auf der Suche nach Schlüsseln zu Privatgemächern und findest folgende Exemplare:`n`n`c');
		/*		
		...
		*/
		...
		if(db_num_rows($res) || $session['user']['housekey']) {
			...
			output('`&Eigene Privatgemächer:`@`n');
			...
			if($session['user']['housekey']) {
				...
				output('`&Eigenes Gemach in eigenem Haus`0: '.create_lnk('Betreten',$link).'`n',true);
				output('`n~~~`n');
			}
			while($i = db_fetch_assoc($res)) {
				...
				output('`&Eigenes Gemach in '.$i['housename'].'`0: '.create_lnk('Betreten',$link).'`n',true);
				output('`n~~~`n');
			}			
		}
		...
		if(db_num_rows($res) == 0) {
			output('`n`iKeine Einladungen in fremde Privatgemächer vorhanden!`i');
		}
		else {
			output('`&Einladungen in fremde Gemächer:`n`@');
			addnav('Einladungen');
