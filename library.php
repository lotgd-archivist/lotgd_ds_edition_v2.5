...
case 'new':
	...
	$result = db_query($sql) or die(db_error(LINK));
	output("<table cellpadding=2 cellspacing=1 bgcolor='#999999'><tr class='trhead'><td>Titel</td><td>Autor</td><td>Thema</td></tr>",true);
	if (db_num_rows($result)==0) {
		output("<tr class='trdark'><td colspan='3'>Es gibt leider bisher noch keine Bücher.</td></tr>",true);
	}
	else {
		addnav('Bücher');
		$bgclass = '';
...
case "book":
	...
	$result = db_query($sql) or die(db_error(LINK));
	while ($row2 = db_fetch_assoc($result)) {
		if ($row2['bookid']!=$_GET['bookid']) addnav($row2['title'],'library.php?op=book&bookid='.$row2['bookid'],false,false,false,false);
