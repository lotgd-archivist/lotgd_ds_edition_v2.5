...
else if ($_GET['op']=="trank2")
{
    ...
    if (is_array($row))
    {
        ...
        $sql = "INSERT INTO commentary (postdate,section,author,comment) VALUES (now(),'$temp',".$session['user']['acctid'].",'/me `tflüstert dem Knecht etwas ins Ohr und dieser serviert ".addslashes($who)." ".$drink." ')";
...
else if ($_GET['op']=="speis")
{
    ...
    if ($essen>0)
    {
        ...
        {
            output("`tEr teilt dir mit, dass ".$who."`t bereits etwas zu Essen bekommen hat und der Teller noch auf dem Tisch steht.");
        }
    }
    else
    {
    ...
    else
    {
        ...
        if (($session['user']['marriedto']==4294967295 || $session['user']['charisma']==4294967295) && ($row['marriedto']==4294967295 || $row['charisma']==4294967295))
        {
            ...
            case 4:
                ...
                `6 wurde gesehen, wie ".($session['user']['sex']?"sie":"er")." mit`&{$row[name]} kichernd im Hinterzimmer der Taverne verschwand und du verlässt ".($session['user']['sex']?"sie":"ihn").".");
            ...
            else
            {
                ...
                case 4:
                    ...
                    if ($session['user']['charisma']==4294967295)
                    {
                        ...
                        db_query($sql);
                        systemmail($session['user']['marriedto'],"`$Scheidung!`0","`&{$session['user']['name']}
                        `6 wurde gesehen, wie ".($session['user']['sex']?"sie":"er")." mit `&{$row[name]} im kichernd ins Hinterzimmer der Taverne verschwand. Du verlässt ".($session['user']['sex']?"sie":"ihn").".");
                    }
                    $session['user']['marriedto']=$row['acctid'];
                    $session['user']['charisma']=1;
                    $session['user']['seenlover']=1;
